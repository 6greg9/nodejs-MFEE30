...
/*
A
    1.2都在then處理reject, 2多了在reject又回傳一個promise, 所以可以繼續
    3.在then 之前用catch 處理reject, catch 之後的then反而沒有接到資料
B
    在then裡面處理 resolve, reject , 用 catch 處理例外 較好(個人意見),
    沒有寫reject要怎樣處理時,會當成catch 處理,
    但catch 也負責 throw 其他的例外,
    reject 也不一定單純吐出錯誤訊息, 可能再跑其他流程,
